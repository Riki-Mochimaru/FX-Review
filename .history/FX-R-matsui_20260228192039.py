...
def _filter_time_range(df: pd.DataFrame, tcol: str, start, start) -> pd.DataFrame:
    series = df[tcol]
...
def make_page1_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start, start):
...
def make_page2_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start, start):
    pnl = trades["pnl"].astype(float).to_numpy()
