...
Example:
python scripts/plot_trade_windows.py \
  --trades data/execution_list_20260306014736.csv \
  --before-minutes 30 \
  --after-minutes 30 \
  --candle-minutes 5 \
  --trades-per-page 4 \
...
def _load_fx_r_matsui_module():
    ...
    module_path = Path(__file__).resolve().parents[1] / "FX-R-matsui.py"
...
def slice_trade_window(
    ...
    """Slice tick and ohlc by [min(entry,exit)-before, max(entry,exit)+after]."""
    ...
    ohlc_w = ohlc[(ohlc.index >= start) & (ohlc.index <= end)].copy()
...
def _bollinger_bands(close: pd.Series, window: int = 20, sigma: float = 2.0) -> pd.DataFrame:
...
def plot_single_trade_column(
    ...
    if not ohlc_w.empty:
        ...
        bb = _bollinger_bands(ohlc_w["close"], window=20, sigma=2.0)
        ax_candle.plot(bb.index, bb["bb_u"], color="tab:blue", linestyle="--", linewidth=0.9, alpha=0.8, label="BB +2σ")
        ax_candle.plot(bb.index, bb["bb_mid"], color="gray", linestyle="-", linewidth=0.7, alpha=0.7, label="BB MA20")
        ax_candle.plot(bb.index, bb["bb_l"], color="tab:blue", linestyle="--", linewidth=0.9, alpha=0.8, label="BB -2σ")
    else:
        bb = pd.DataFrame()
...
def save_trade_window_pages(
    ...
    """Render paginated trade-window figures and save PNG + PDF."""
    ...
    for page_idx, page_trades in enumerate(pages, start=1):
        ...
        png = outdir / f"{base}_page{page_idx:02d}.png"
    ...
    with PdfPages(pdf_path) as pdf:
        for png in png_paths:
            ...
            h, w = img.shape[:2]
            fig = plt.figure(figsize=(w / 110.0, h / 110.0))
    ...
    return png_paths, pdf_path, rendered, skipped
...
def main() -> None:
    ...
    pngs, pdf_path, rendered, skipped = save_trade_window_pages(
        trades=trades,
        ticks=ticks,
