...
# ===== Plotting =====
def draw_candles(ax, ohlc: pd.DataFrame, width: float = 0.6, start: Optional[pd.Timestamp] = None, end: Optional[pd.Timestamp] = None):
...
def make_page1_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start: pd.Timestamp, end: pd.Timestamp):
...
def make_page2_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start: pd.Timestamp, end: pd.Timestamp):
    pnl = trades["pnl"].astype(float).to_numpy()
