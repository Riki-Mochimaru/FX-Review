...
def _filter_time_range(df: pd.DataFrame, tcol: str, start, end: pd.Timestamp) -> pd.DataFrame:
    series = df[tcol]
...
# ===== Plotting =====
def draw_candles(ax, ohlc: pd.DataFrame, width: float = 0.6, start: int, end: int):
...
def make_page1_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start, end: pd.Timestamp):
    ...
    draw_candles(ax1, ohlc, trades, start, end)
...
def make_page2_png(metrics: dict, trades: pd.DataFrame, ohlc: pd.DataFrame, out_png: Path, start, end: pd.Timestamp):
    pnl = trades["pnl"].astype(float).to_numpy()
